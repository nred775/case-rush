...
setTimeout(() => setToastMessage(""), 3000);
  ...
  return (
    <div className="text-white max-w-4xl mx-auto px-8 py-6">
...
{leaders
  ...
  return (
    <li
  key={user.id}
  className={`p-3 sm:p-4 rounded-lg flex flex-col sm:flex-row sm:justify-between items-start sm:items-center gap-2 sm:gap-8 transition-all duration-300 ${
    i > 2 ? "bg-gray-900 border border-gray-700 hover-glow" : ""
  } ${isCurrent ? "ring-4 ring-blue-950 bg-blue-950/60" : ""}`}


      style={
        i === 0
          ? { backgroundColor: "#1f1f0f", border: "2px solid #facc15" }
          : i === 1
          ? { backgroundColor: "#1f1f1f", border: "2px solid #d1d5db" }
          : i === 2
          ? { backgroundColor: "#2a1a0f", border: "2px solid #fdba74" }
          : {}
      }
    >
          ...
          <button onClick={() => handleClickUser(user)}>
            {user.equippedAvatar ? (
              <img
                ...
                className={`w-14 h-14 sm:w-16 sm:h-16 rounded-full border-4 bg-black/30 hover:scale-110 transition-transform ${getLevelBorderClass(user.level || 1)}`}
              />
            ) : (
